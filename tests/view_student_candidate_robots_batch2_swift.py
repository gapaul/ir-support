"""Open the second student candidate robot batch in Swift.
...
SCREENSHOT_PATH = Path(__file__).resolve().parent / "_swift_screenshots" / "student_candidate_robots_batch2.png"
ANIMATION_STEPS = 70
...
def save_swift_screenshot():
    """Save the current browser view for visual inspection."""
...
def joint_wave_ranges(robot, start_q):
    ...
    for index, link in enumerate(robot.links):
        ...
        if getattr(link, "isprismatic", False):
            desired = PRISMATIC_JOINT_AMPLITUDE
        else:
            desired = REVOLUTE_JOINT_AMPLITUDE
...
def main():
    ...
    print("Animating student candidate robot batch 2 in Swift...")
