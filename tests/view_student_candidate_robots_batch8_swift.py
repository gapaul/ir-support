"""
Open the eighth student candidate robot batch in Swift.

Run this file directly from the IDE, for example with Ctrl+F5.
The pytest file `test_student_candidate_robots_batch8.py` stays headless and fast.
"""
...
from ir_support_extra_robots.robots.UR20.UR20 import UR20  # noqa: E402
from ir_support_extra_robots.robots.YaskawaHC20DTP.YaskawaHC20DTP import YaskawaHC20DTP  # noqa: E402
from ir_support_extra_robots.robots.PAROL6.PAROL6 import PAROL6  # noqa: E402
from ir_support_extra_robots.robots.ABBIRB1300.ABBIRB1300 import ABBIRB1300  # noqa: E402


ROBOT_FACTORIES = [
    ("UR20", UR20),
    ("YaskawaHC20DTP", YaskawaHC20DTP),
    ("PAROL6", PAROL6),
    ("ABBIRB1300", ABBIRB1300),
]
...
def robot_label(index, label):
    row, col = grid_position(index)
    return f"{index + 1}. {label} (row {row + 1}, col {col + 1})"
...
def robot_camera_pose(robot, index):
    ...
    distance = min(max(span * 1.7, DEFAULT_CAMERA_DISTANCE), 6.0)
    look_at = base_position + np.array([0.0, 0.0, min(max(span * 0.35, 0.25), 1.8)])
    position = base_position + np.array([distance, -distance, min(max(distance * 0.55, 0.8), 3.4)])
...
def add_robot_buttons(env, robots):
    if HEADLESS:
        return
    env.add(swift.Label("Student candidate robots, batch 8"))
    for index, ((label, _), robot) in enumerate(zip(ROBOT_FACTORIES, robots)):
        env.add(
            swift.Button(
                lambda _, robot=robot, index=index: set_robot_camera(env, robot, index),
                desc=robot_label(index, label),
            )
        )
...
def main():
    ...
    for index, (label, factory) in enumerate(ROBOT_FACTORIES):
        ...
        print(f"Loaded {robot_label(index, label)}: {robot.n} joints, {len(robot.links_3d)} meshes")

    add_robot_buttons(env, robots)
    print("Animating student candidate robot batch 8 in Swift...")
