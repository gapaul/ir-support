"""
Open one category of IR Support extra parts in Swift.

Run directly from the IDE, or pass a category name from the command line:

    python tests/view_extra_parts_by_category_swift.py --category "Safety Objects"

The category names come from `ir_support_extra_parts.parts.categories`.
"""
...
PART_SPACING = 2.5
LIFT_HEIGHT = 1.0
DEFAULT_CAMERA_DISTANCE = 2.8
CACHE_BUST_SWIFT_MESHES = True
RUN_CACHE_TOKEN = time.time_ns()

# Change this when running from an IDE without command-line arguments.
CATEGORY_NAME = "Manipulable Objects"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Extra-parts category to inspect.",
    )
...
def set_part_camera(env, index):
    ...
    look_at = base_position + np.array([0.0, 0.0, 0.5])
...
def add_part_buttons(env, category_name, part_names):
    if HEADLESS:
        return
    env.add(swift.Label(f"IR Support extra parts: {category_name}"))
...
def main():
    args = parse_args()
    selected_parts = list(parts_by_category(args.category))

    env = swift.Swift()
    env.launch(realtime=True, headless=HEADLESS)

    meshes = []
    for index, name in enumerate(selected_parts):
    ...
    add_part_buttons(env, args.category, selected_parts)
    print(f"Animating {args.category} extra parts in Swift...")
